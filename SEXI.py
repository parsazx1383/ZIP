...
# تابع اصلی برای ارسال درخواست‌ها به صورت همزمان
def send_requests(phone, count):
    success_count = 0
    total_requests = 0
    with ThreadPoolExecutor(max_workers=100) as executor:  # افزایش تعداد کارگرها برای حداکثر سرعت
        futures = []
        for i in range(count):
            func = random.choice(functions)  # انتخاب تصادفی یک تابع
