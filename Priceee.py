...
#==================== Database Functions =====================#
def get_data(query):
    with pymysql.connect(host="127.0.0.1", port=3307, database=DBName, user=DBUser, password=DBPass, cursorclass=pymysql.cursors.DictCursor) as connect:
        db = connect.cursor()
        db.execute(query)
        result = db.fetchone()
        return result
...
def get_datas(query):
    with pymysql.connect(host="127.0.0.1", port=3307, database=DBName, user=DBUser, password=DBPass) as connect:
        db = connect.cursor()
        db.execute(query)
        result = db.fetchall()
        return result
...
def helper_getdata(query):
    with pymysql.connect(host="127.0.0.1", port=3307, database=HelperDBName, user=HelperDBUser, password=HelperDBPass) as connect:
        db = connect.cursor()
        db.execute(query)
        result = db.fetchone()
        return result
